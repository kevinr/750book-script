...
import sys, re, calendar, math, optparse, getpass
from datetime import datetime

from mako.template import Template


DEFAULT_TITLE = "750 Words Morning Pages"
...
# default title is supplied by render(), default author (being system-dependent) is supplied by main()
def main():
    global DEFAULT_TITLE
    default_author = get_fullest_user_name()

    usage = "Usage: %prog [-t|--title=TITLE] [-a|--author=AUTHOR] [-o|--output=FILE] INPUT[..]"
    parser = optparse.OptionParser(usage=usage)
    parser.add_option('-t', '--title', 
        help='title of the book, default "%default"', 
        default=DEFAULT_TITLE,
        metavar='"TITLE"')
    parser.add_option('-a', '--author', 
        help='author of the book, default "%default"', 
    ...
    inputs = [open(filename) for filename in args]
    ...
    output.write(render(*inputs, title=options.title, author=options.author).encode('utf-8'))
...
def render(*inputs, **kwargs):
    ...
    title = 'title' in kwargs and kwargs['title'] or DEFAULT_TITLE
...
def get_fullest_user_name():
    try:
        # pwd doesn't exist on Windows, but on Linux, I can get the user's full name
        # from the pw_gecos field of the passwd database.  I see a ctypes hack that
        # theoretically works on Windows at 
        # http://sjohannes.wordpress.com/2010/06/19/win32-python-getting-users-display-name-using-ctypes/
        # but somebody else gets to make that happen.
        import pwd
        return pwd.getpwnam(getpass.getuser())[4].strip(',')
    except ImportError:
        pass
